const calculatorPro = () => {
    ...
    if (numberList.length == 1 && sign == '') {
            prompt('the square root of this number is ' + Math.sqrt(numberList[0]))
        }
 ...
 for (let i = 1; i < numberList.length; i++) {
    ...
    prompt( 'The result of this sum is ' + resultSum.toFixed(2));
         } else if (sign == '-') {
            resultRest -= numberList[i];
    prompt('The result of this subtraction is ' + resultRest.toFixed(2));        
         } else if (sign == '*') { 
            resultMultiply *= numberList[i]; 
    prompt( 'The result of this multiplication is ' + resultMultiply.toFixed(2));         
         } else if (sign == '/') { 
            resultDivide /= numberList[i];
    prompt('The result of this division is  ' + resultDivide.toFixed(2)); 
         }
